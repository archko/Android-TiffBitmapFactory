...
jobject NativeDecoder::getBitmap(jstring path, jint fd, jobject opts, jobject listener) {
    jPath = path;
    jFd = fd;
    optionsObject = opts;
    listenerObject = listener;
    ...
    if (setjmp(NativeDecoder::general_buf)) {
        ...
        LOGE("NativeDecoder error=%s", err);
        if (throwException) {
    ...
    if (image == NULL) {
        ...
        if (decodingMode == DECODE_MODE_FILE_PATH) {
            LOGE("Can\'t open bitmap:%s", strPath);
            env->ReleaseStringUTFChars(jPath, strPath);
        } else {
            LOGE("Can\'t open file descriptor:%s", jFd);
        }
        return NULL;
    } else {
        if (decodingMode == DECODE_MODE_FILE_PATH) {
    ...
    if (decodeArea) {
        ...
        if (boundX >= origwidth - 1) {
            ...
            LOGE("boundX >= origwidth:%s", *message);
            if (throwException) {
                throwDecodeFileException(message);
            }
        ...
        if (boundY >= origheight - 1) {
            ...
            LOGE("boundY >= origheight:%s", *message);
            if (throwException) {
                throwDecodeFileException(message);
            }
            env->DeleteLocalRef(decodeAreaClass);
...
NativeDecoder::getSampledRasterFromStrip(int inSampleSize, int *bitmapwidth, int *bitmapheight) {
    ...
    LOGI("width:%d, h:%d", origwidth, origheight);
    ...
    *bitmapheight = origheight / inSampleSize;
    ...
    LOGI("new width:%d h:%d", *bitmapwidth, *bitmapheight);
    ...
    } else if (origorientation == 2 || origorientation == 3 ||
               origorientation == 6 || origorientation == 7) {
        flipPixelsHorizontal(*bitmapwidth, *bitmapheight, pixels);
    }
    ...
    for (int i = 0; (i < stripMax * rowPerStrip || i > boundY + boundHeight); i += rowPerStrip) {
        ...
        if (inSampleSize == 1) {
            ...
            for (int resBmpY = workWritedLines, workY = 0;
                 resBmpY < *bitmapheight &&
                 workY < rowPerStrip; /*wj++,*/ workY++/*= inSampleSize*/) {
                ...
                if (globalLineCounter % inSampleSize == 0) {
                    for (int resBmpX = 0, workX = 0;
                         resBmpX < *bitmapwidth; workX += inSampleSize) {
...
NativeDecoder::getSampledRasterFromImage(int inSampleSize, int *bitmapwidth, int *bitmapheight) {
    ...
    long origBufferSize = origwidth * origheight * sizeof(unsigned int);
    ...
    *bitmapheight = origheight / inSampleSize;
    ...
    if (origBuffer == NULL) {
        LOGE("getSampledRasterFromImage.Can\'t allocate memory for origBuffer:%d", origBufferSize);
        return NULL;
    }
    ...
    if (0 ==
        ...
        LOGE("TIFFReadRGBAImageOriented:%s", *message);
        if (throwException) {
            throwDecodeFileException(message);
        }
        return NULL;
...
jint *NativeDecoder::getSampledRasterFromImageWithBounds(int inSampleSize, int *bitmapwidth,
    ...
    *bitmapheight = boundHeight / inSampleSize;//origheight / inSampleSize;
    ...
    origBuffer = (unsigned int *) _TIFFmalloc(origBufferSize);
    if (origBuffer == NULL) {
        LOGE("Can\'t allocate memory for origBuffer:%s", origBufferSize);
        return NULL;
    }
...
int NativeDecoder::getDecodeMethod() {
    ...
    if (tileWidth > 0 && tileHeight > 0 && readTH > 0 && readTW > 0) {
        ...
        LOGI("RPS:%d, stripSize:%d, stripMax:%d", rowPerStrip, stripSize, stripMax);
...
void NativeDecoder::writeDataToOptions(int directoryNumber) {
    ...
    */
    ...
    LOGI("xres:%d, yres:%d", xresolution, yresolution);
    ...
    if (tagRead == 1) {
        LOGI("bit per sample:%d", bitPerSample);
    ...
    if (tagRead == 1) {
        LOGI("sample per pixel:%d", samplePerPixel);
    ...
    if (tagRead == 1) {
        LOGI("row per strip:%d", rowPerStrip);
    ...
    //strip max
    uint32 stripMax = TIFFNumberOfStrips(image);
    LOGI("number of strips:%d", stripMax);
